...
def main():
    ...
    tests = [(re.sub(r' \(test-.*', '', re.sub(':[^:]*: ', '', step['label'])),
              step['command'],
             ...
             and not step['label'].startswith(':docker: Build ')
             for plugin in step['plugins'] if 'docker-compose#v3.5.0' in plugin]

    # we need to distinguish the two oneccl variants of some tests
    tests = [(label + (' [ONECCL OFI]' if 'mpirun_command_ofi' in command else (' [ONECCL MPI]' if 'mpirun_command_mpi' in command else '')),
              command,
              timeout,
              image)
             for label, command, timeout, image in tests]

    # check that labels are unique per image
    cardinalities = Counter([(label, image) for label, command, timeout, image in tests])
    ...
    tests = [(new_labels_per_label_command[(label, command)] if (label, command) in new_labels_per_label_command else label,
              command,
              timeout,
              image)
             for label, command, timeout, image in tests]
    ...
    test_labels = {label for label, command, timeout, image in tests}
    ...
    tests_per_image = {image: {test for test in [test_id_per_label[label]
                                                 for label, command, timeout, test_image in tests
    ...
    def publish_docker_images(needs: List[str], images: List[str]) -> str:
        if 'init-workflow' not in needs:
            needs.insert(0, 'init-workflow')
        if needs != ['init-workflow', 'build-and-test']:
        ...
        return (f'  docker-config:\n'
                ...
                f'    needs: [{", ".join(needs)}]\n'
                f"    # build-and-test-cpu and build-gpu might have been skipped (! needs.init-workflow.outputs.run_builds_and_tests)\n"
    ...
    with open(path.joinpath('workflows', 'ci.yaml').absolute(), 'wt') as w:
        ...
        release_images = [cpu_release_images[0], gpu_release_images[0]]
        ...
        workflow = workflow_header() + jobs(
            ...
            publish_docker_images(needs=['build-and-test'], images=['horovod', 'horovod-cpu', 'horovod-ray']),
